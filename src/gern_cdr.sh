...
indb#5100
"
filter1_dir="${BOSS_DATA5}/pp_yw/std/fixvoice/it100/" #一次分拣输入目录
spool_sh_dir="/ngbss/billing5/user/gucb/spool_cdr" #话单导出脚本输入路径
spool_file="filter_fix_gsm_spool.sh" #生成话单脚本的文件名
g_select_cond="ucr_hen01.tg_cdr_fix_special_201407 where source_type not in ('9H','9L')" #生成话单的表和条件
...
cd ~/user/gucb/
sh balance_acct.sh
writeLog " 对账完成，请查询对账结果 "
}

check_count(){
  ct=$(get_data "$db_user" "select count(*) from ${t_cbss};")
  while [ "${ct}" -lt "${all_ct}" ]
  do
    writeLog "现在已入库 ${ct} 条"
    sleep 40
