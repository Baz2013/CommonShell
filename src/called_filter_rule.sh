...
get_rule(){
  called_count=$(echo $2|awk 'BEGIN{FS="/"; i=0;} {i=i+NF} END{print i;}') #»»³É awk -F"/" '{print NF}' ¸ü¼ò½à
  file_count=$(echo $3|awk 'BEGIN{FS="/"; i=0;} {i=i+NF} END{print i;}') #awk -F"/" '{print NF}'
  echo "$called_count $file_count"
  i=1
  str1=""
  if [ $called_count -eq 1 ];then
    len=$(expr length ${2})
    str1="term_nbr_pre${len}=${2}"
  elif [ $called_count -gt 1 ];then
  while [ $i -le $called_count ]
  do
    num1=$(echo $2|awk -F"/" '{print $v}' v="$i")
    len=$(expr length $num1)
    str1="(term_nbr_pre${len}=${num1})OR${str1}"
    let i=i+1
  done
  fi
...
ENDIF
IF((tmp_bj05_flag${flag1}=1)AND(home_area_code=${1})AND(tmp_file_flag${flag2}=1))
THEN
    RULE_2:CALL_TYPE=FF,ERROR_CODE=570
ENDIF " >>called_filter3.cfg
let flag1=flag1+1
let flag2=flag2+1
}
...
sed "s/)OR)/))/g" < called_filter3.cfg > called_filter3.cfg3
